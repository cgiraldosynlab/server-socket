...
class ClientSocket:
    ...
    def __init__(self):
        try:
            ...
            self.__buffer = (int(config.global_enviroment['SOCKET_SERVER_BUFFER']) if config.global_enviroment.get('SOCKET_SERVER_BUFFER') else 1024)
    ...
    def send_data(self, content: str) -> bool:
        try:
            ...
            log_show(msg=response.decode().replace(chr(11), '').replace(chr(28), ''), level='info', procedure='_send_data', file=__class__)
            client.close()
                ...
                if os.path.exists(name_path):
                    with open(name_path) as file:
                        try:
                            ...
                            print(dict(enumerate(content[0:100])))
                            if content == '':
